<template>
  <div class="progress min-h-16 border rounded-lg shadow-sm py-2 px-4 hover:bg-gray-50 active:bg-gray-100"
    :class="[bgColor, (selectionMode && props.isPaused) ? 'opacity-50 cursor-not-allowed bg-gray-100' : '']"
    @click="(selectionMode && !props.isPaused) ? $emit('toggleSelect') : (!selectionMode ? $emit('openDetail') : null)">

    <div class="flex flex-row items-center">
      <!-- Checkbox visible only in selection mode -->
      <div v-if="selectionMode && !props.isPaused" class="mr-4">
        <!-- Custom circular checkbox -->
        <label class="flex items-center cursor-pointer relative">
          <input type="checkbox" :checked="isSelected" @change.stop="$emit('toggleSelect')"
      ...
      <div class="flex flex-col flex-grow">
        <div class="flex justify-between items-center gap-2" style="min-height: 26px;">
          ...
          <div class="flex flex-col">
            ...
            <span v-if="selectionMode && props.isPaused" class="text-xs text-red-500 font-semibold mt-1">Habit is
              paused</span>
          </div>
        ...
        <div class="relative h-2 mt-2 bg-gray-200 rounded-full overflow-hidden">
          <div :class="color" class="absolute h-full left-0 top-0 rounded-full" :style="{ width: `${percent}%` }"></div>
        </div>
      </div>
    </div>
